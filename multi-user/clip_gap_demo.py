...
class ImageEncoder(Protocol):
    ...
    def encode(self, images: list[Any]) -> list[list[float]]:
        ...
...
TEMPORAL_KMEANS_DISTANCE_FORMULA = (
...
def temporal_spherical_kmeans_medoids(
    ...
    *,
    ...
    """Return deterministic time-aware spherical K-means labels and medoids.

    This is the production version of the validated temporal-K-means sidecar.
    Assignment maximizes cosine similarity minus a quadratic, normalized time
    penalty.  ``time_weight=0`` is exactly compatible with the legacy cosine
    implementation.
    """

    vectors = np.asarray(embeddings, dtype=np.float64)
    times = np.asarray(timestamps_seconds, dtype=np.float64)
    if vectors.ndim != 2 or len(vectors) == 0 or vectors.shape[1] == 0:
        raise ValueError("embeddings must form a non-empty two-dimensional matrix")
    if times.ndim != 1 or len(times) != len(vectors):
        raise ValueError("timestamps must align one-to-one with embeddings")
    if not np.isfinite(vectors).all() or not np.isfinite(times).all():
        raise ValueError("embeddings and timestamps must contain only finite values")
    if cluster_count <= 0:
    ...
    while len(center_indices) < k:
        ...
        candidates[np.asarray(center_indices, dtype=np.int64)] = -np.inf
        center_indices.append(int(np.argmax(candidates)))
    ...
    for iteration in range(max_iterations):
        ...
        new_labels = np.argmax(visual_similarity - temporal_penalty, axis=1).astype(
            np.int64
        )
        new_labels[np.asarray(center_indices, dtype=np.int64)] = np.arange(
            k, dtype=np.int64
        )
        ...
        for cluster_index in range(k):
            ...
            new_temporal_centers[cluster_index] = float(
                scaled_times[members].mean()
            )
    ...
    return labels.tolist(), medoids, {
        ...
        "distance_formula": TEMPORAL_KMEANS_DISTANCE_FORMULA,
        ...
        "temporal_center_seconds": [
            round(float(times[0] + center * temporal_unit_seconds), 6)
...
class TransformersClipEncoder:
    ...
    def encode(self, images: list[Any]) -> list[list[float]]:
        """Encode paths or already-decoded RGB images without a disk round trip."""

        prepared = [
            self.image_class.open(image).convert("RGB")
            if isinstance(image, (str, Path))
            else image.convert("RGB")
            if hasattr(image, "convert")
            else self.image_class.fromarray(np.asarray(image, dtype=np.uint8), mode="RGB")
            for image in images
        ]
...
def _read_ppm_token(stream: BinaryIO) -> bytes | None:
...
def _read_exact(stream: BinaryIO, size: int) -> bytes:
    ...
    if len(chunks) != size:
        raise RuntimeError(
            f"ffmpeg RGB stream ended early: expected={size} actual={len(chunks)}"
        )
    return bytes(chunks)


def _decode_ffmpeg_ppm_stream(
    command: list[str],
    *,
    ...
    try:
        ...
        raise RuntimeError("Pillow is required for direct RGB video sampling") from exc
    ...
    if process.stdout is None:
        raise RuntimeError("ffmpeg did not expose its RGB output pipe")
    frames: list[Any] = []
    try:
        while True:
            magic = _read_ppm_token(process.stdout)
            if magic is None:
                break
            if magic != b"P6":
                raise RuntimeError(f"unexpected ffmpeg PPM magic: {magic!r}")
            width_token = _read_ppm_token(process.stdout)
            height_token = _read_ppm_token(process.stdout)
            max_value_token = _read_ppm_token(process.stdout)
            if None in (width_token, height_token, max_value_token):
                raise RuntimeError("incomplete ffmpeg PPM header")
            width = int(width_token)
            height = int(height_token)
            max_value = int(max_value_token)
            if width <= 0 or height <= 0 or max_value != 255:
                raise RuntimeError(
                    f"unsupported ffmpeg PPM frame: {width}x{height} max={max_value}"
                )
            pixels = _read_exact(process.stdout, width * height * 3)
            frames.append(Image.frombytes("RGB", (width, height), pixels))
    finally:
    ...
    minimum = min(expected_frame_count, max(1, expected_frame_count - 2))
    if len(frames) < minimum:
        raise RuntimeError(
            "ffmpeg direct RGB sampling returned too few frames: "
            f"source={source_description} expected={expected_frame_count} "
            f"minimum={minimum} actual={len(frames)}"
        )
    print(
        "sampled_video_rgb "
        f"status=decoded source={source_description} frames={len(frames)} "
...
def decode_video_rgb_frames(
    ...
    *,
    ...
    """Decode the existing ffmpeg FPS sampling directly into RGB PIL images.

    PPM image2pipe keeps frame boundaries self-describing while avoiding both
    an assembled long video and intermediate PNG files.
    """
    ...
    if not ffmpeg:
        ...
        if explicit.exists():
            ffmpeg = str(explicit)
    if not ffmpeg:
        raise RuntimeError("ffmpeg is required for direct RGB video sampling")
    expected = max(1, int(math.ceil(duration_seconds / sample_interval_seconds)))
    command = [
        ffmpeg,
        "-hide_banner",
        "-loglevel",
        ...
        "-vf",
        f"fps={1.0 / float(sample_interval_seconds):.12g}:start_time=0,format=rgb24",
        "-frames:v",
        str(expected),
        "-f",
        "image2pipe",
    ...
    return _decode_ffmpeg_ppm_stream(
        command,
        expected_frame_count=expected,
        source_description=str(video_path),
    )


def decode_source_segments_rgb_frames(
    ...
    *,
    ...
    """Continuously sample ordered clips with one padded concat-filter process."""
    ...
    if not ffmpeg:
        raise RuntimeError("ffmpeg is required for direct RGB video sampling")
    resolved = [Path(path).resolve() for path in video_paths]
    nominal_durations = (
        [float(value) for value in segment_durations_seconds]
        if segment_durations_seconds is not None
        else [None for _ in resolved]
    )
    if len(nominal_durations) != len(resolved) or any(
        value is not None and value <= 0 for value in nominal_durations
    ):
        raise ValueError("nominal segment durations must be positive and align with videos")
    for path in resolved:
        if not path.is_file():
            raise FileNotFoundError(f"source segment is missing: {path}")
    expected = max(1, int(math.ceil(duration_seconds / sample_interval_seconds)))
    input_args = [value for path in resolved for value in ("-i", str(path))]
    prepared_labels = []
    filters = []
    for index, nominal_duration in enumerate(nominal_durations):
        label = f"v{index}"
        prepared_labels.append(f"[{label}]")
        chain = f"[{index}:v]setpts=PTS-STARTPTS"
        if nominal_duration is not None:
            # Pad each logical 30-second slot before concatenation.  This keeps
            # later users/times aligned even when an encoded MP4 ends slightly
            # early, without writing a repaired intermediate video.
            chain += (
                f",tpad=stop_mode=clone:stop_duration={nominal_duration:.6f}"
                f",trim=duration={nominal_duration:.6f}"
            )
        filters.append(f"{chain}[{label}]")
    filters.append(
        "".join(prepared_labels)
        + f"concat=n={len(resolved)}:v=1:a=0,"
        + f"fps={1.0 / float(sample_interval_seconds):.12g}:start_time=0,"
        + "format=rgb24[outv]"
    )
    command = [
        ffmpeg,
    ...
    return _decode_ffmpeg_ppm_stream(
        command,
        expected_frame_count=expected,
        source_description=f"{len(resolved)}_ordered_segments",
    )


def sample_source_segments_rgb(
    ...
    *,
    ...
    sample_interval_seconds: float,
    ffmpeg_binary: str = "ffmpeg",
    persist_png_compress_level: int = 1,
    persist_format: str = "png",
    persist_jpeg_quality: int = 95,
    attach_rgb_images: bool = True,
) -> list[dict[str, Any]]:
    """Sample ordered source segments directly and persist only final images.

    PNG remains the lossless default for the existing QA path.  Offline dataset
    builders can request JPEG and omit the attached RGB objects so CLIP reloads
    the exact durable representation that downstream generation will consume.
    """

    if not source_segments:
        raise ValueError("direct segment sampling requires ordered source segments")
    normalized_format = str(persist_format).strip().lower()
    if normalized_format == "jpg":
        normalized_format = "jpeg"
    if normalized_format not in {"png", "jpeg"}:
        raise ValueError("persist_format must be 'png' or 'jpeg'")
    if not 1 <= int(persist_jpeg_quality) <= 100:
        raise ValueError("persist_jpeg_quality must be between 1 and 100")
    sample_started = time.monotonic()
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    ordered = sorted(
        source_segments,
        key=lambda row: int(row.get("segment_index", 0)),
    )
    segment_windows: list[tuple[float, float, int, Path]] = []
    for fallback_index, segment in enumerate(ordered):
        segment_index = int(segment.get("segment_index", fallback_index))
        window_start = float(
            segment.get("window_start_seconds", fallback_index * 30.0)
        )
        window_end = min(
            float(duration_seconds),
            float(segment.get("window_end_seconds", window_start + 30.0)),
        )
        if window_end <= window_start:
            continue
        video_path = Path(str(segment.get("local_video") or ""))
        if not video_path.is_file():
            raise FileNotFoundError(f"source segment is missing: {video_path}")
        segment_windows.append((window_start, window_end, segment_index, video_path))
    if not segment_windows:
        raise ValueError("direct segment sampling found no positive-duration segments")

    timestamps = [
        index * float(sample_interval_seconds)
        for index in range(max(1, int(math.ceil(duration_seconds / sample_interval_seconds))))
        if index * float(sample_interval_seconds) < float(duration_seconds) - 1e-9
    ]

    def source_segment_index(timestamp: float) -> int:
        for window_start, window_end, segment_index, _ in segment_windows:
            if window_start - 1e-9 <= timestamp < window_end - 1e-9:
                return segment_index
        return segment_windows[-1][2]

    suffix = ".png" if normalized_format == "png" else ".jpg"
    paths = [
        output_dir / f"frame_{index:04d}_{timestamp:.2f}s{suffix}"
        for index, timestamp in enumerate(timestamps)
    ]
    if paths and all(path.is_file() and path.stat().st_size > 0 for path in paths):
        print(
            "sampled_video_rgb "
            f"status=cache_hit source={len(segment_windows)}_ordered_segments "
            f"frames={len(paths)} ffmpeg_processes=0 "
            f"total_seconds={time.monotonic() - sample_started:.3f}",
            flush=True,
        )
        return [
            {
                ...
                "path": str(path),
                "source_segment_index": source_segment_index(timestamp),
    ...
    images = decode_source_segments_rgb_frames(
        [window[3] for window in segment_windows],
        duration_seconds=duration_seconds,
        sample_interval_seconds=sample_interval_seconds,
        segment_durations_seconds=[
            window_end - window_start
            for window_start, window_end, _, _ in segment_windows
        ],
    ...
    for index, (timestamp, image) in enumerate(zip(timestamps, images)):
        path = paths[index]
        ...
        if normalized_format == "png":
            image.save(
                temporary,
                format="PNG",
                compress_level=int(persist_png_compress_level),
            )
        else:
            image.save(
                temporary,
                format="JPEG",
                quality=int(persist_jpeg_quality),
                subsampling=0,
            )
        ...
        if attach_rgb_images:
            row["_rgb_image"] = image
        else:
            image.close()
        rows.append(row)
    print(
        "sampled_video_rgb "
        f"status=persisted source={len(segment_windows)}_ordered_segments "
        f"frames={len(rows)} format={normalized_format} "
        f"quality={persist_jpeg_quality if normalized_format == 'jpeg' else 'lossless'} "
        f"decode_seconds={decode_seconds:.3f} "
        f"persist_seconds={time.monotonic() - persist_started:.3f} "
        f"total_seconds={time.monotonic() - sample_started:.3f}",
        flush=True,
    )
    return rows
