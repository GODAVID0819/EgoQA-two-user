...
def hydrate_deferred_gaze_summaries(packet: dict[str, Any]) -> dict[str, Any]:
    """Resolve gaze summaries only when a candidate reaches generation."""

    if not any(
        bool(clip.get("gaze_summary_deferred"))
    ...
    clips = []
    for clip in packet.get("clips") or []:
        if not isinstance(clip, dict) or not clip.get("gaze_summary_deferred"):
            clips.append(clip)
            continue
        next_clip = dict(clip)
        source_rows = []
        for segment in clip.get("source_segments") or []:
            next_segment = dict(segment)
            gaze_path = Path(str(next_segment.get("local_gaze") or ""))
            calibration_value = next_segment.get("local_calibration")
            calibration_path = Path(str(calibration_value)) if calibration_value else None
            if gaze_path.is_file():
                next_segment["gaze_summary"] = summarize_gaze_csv(
                    gaze_path,
                    calibration_path=calibration_path,
                )
            next_segment["gaze_summary_deferred"] = False
            source_rows.append(next_segment)
        next_clip["source_segments"] = source_rows
        next_clip["gaze_summary"] = _combined_gaze_summary(source_rows)
        next_clip["gaze_summary_deferred"] = False
...
def _prepare_evidence_clip(
    ...
    *,
    ...
    download_media: bool,
    download_gaze: bool,
    assemble_long_video: bool,
    defer_gaze_summary: bool,
    extract_preview_frames: bool,
) -> dict[str, Any]:
    """Download, assemble, and probe one user's evidence independently."""
    ...
    for segment_index, source_clip in enumerate(source_clips):
        ...
        if download_media and download_gaze:
        ...
        gaze_summary = {}
        if not defer_gaze_summary and _nonempty_file(source_local_gaze):
            gaze_summary = summarize_gaze_csv(
                source_local_gaze, calibration_path=calibration_path
            )
        source_rows.append(
            {
                ...
                "window_start_seconds": round(
                    segment_index * SOURCE_CLIP_DURATION_SECONDS, 3
                ),
                "window_end_seconds": round(
                    min(
                        (segment_index + 1) * SOURCE_CLIP_DURATION_SECONDS,
                        evidence_duration,
                    ),
                    3,
                ),
                ...
                "gaze_summary_deferred": bool(
                    defer_gaze_summary and _nonempty_file(source_local_gaze)
                ),
            }
        )
    ...
    if len(source_rows) == 1:
        local_video = (
            Path(source_rows[0]["local_video"])
            if source_rows[0]["local_video"]
            else None
        )
    elif assemble_long_video:
    ...
    duration = ffprobe_duration(local_video) if local_video is not None else evidence_duration
    frame_rows: list[dict[str, Any]] = []
    if extract_preview_frames and local_video is not None:
    ...
    gaze_summary = (
        {}
        if defer_gaze_summary
        else _combined_gaze_summary(source_rows)
    )
    ...
    return {
        ...
        "frames": frame_rows,
        "gaze_summary": gaze_summary,
        "gaze_summary_deferred": bool(defer_gaze_summary),
        "long_video_materialized": bool(local_video is not None),
    }
...
def build_evidence_packet(
    ...
    *,
    ...
    download_gaze: bool = True,
    rng: random.Random | None = None,
    media_prepare_workers: int = 1,
    assemble_long_video: bool = True,
    defer_gaze_summary: bool = False,
    ...
    worker_count = min(media_prepare_workers, len(selected))
    media_started = time.monotonic()
    if worker_count <= 1:
        clips_out = [prepare_clip(clip) for clip in selected]
    else:
        print(
            "evidence_window status=parallel_user_media_preparation "
            f"workers={worker_count} users={len(selected)}",
            flush=True,
        )
        with ThreadPoolExecutor(
            max_workers=worker_count,
            thread_name_prefix="egoqa-evidence-user",
        ) as executor:
            # executor.map preserves the deterministic selected-user order.
            clips_out = list(executor.map(prepare_clip, selected))
        print(
            "evidence_window status=parallel_user_media_preparation_completed "
            f"workers={worker_count} users={len(selected)} "
            f"seconds={time.monotonic() - media_started:.3f}",
            flush=True,
        )
...
def iter_evidence_packets(
    *,
    ...
    skip_incomplete_groups: bool = False,
    media_prepare_workers: int = 1,
    assemble_long_video: bool = True,
    defer_gaze_summary: bool = False,
    extract_preview_frames: bool = True,
    ...
    if manifest_cache_dir is not None:
        ...
        digest = hashlib.sha256(
            manifest_bytes
            + f"\0duration={float(evidence_duration_seconds):.6f}".encode("ascii")
        ).hexdigest()
        manifest_cache_path = (
            Path(manifest_cache_dir) / "manifest_groups" / f"{digest}.json"
        )
        if manifest_cache_path.is_file():
            cached = read_json(manifest_cache_path)
            if isinstance(cached, dict) and isinstance(cached.get("groups"), list):
                groups = cached["groups"]
                if progress:
                    print(
                        "evidence_manifest status=cache_hit "
                        f"groups={len(groups)} path={manifest_cache_path}",
                        flush=True,
                    )
    if groups is None:
        ...
        if manifest_cache_path is not None:
            ...
            if progress:
                print(
                    "evidence_manifest status=cache_written "
                    f"groups={len(groups)} path={manifest_cache_path}",
                    flush=True,
                )
    excluded = {
    ...
    for group_index, group in enumerate(groups):
        ...
        available_users = len(group.get("clips") or [])
        if skip_incomplete_groups and available_users < users_per_case:
            if progress:
                print(
                    "evidence_window "
                    ...
                    f"available_users={available_users} required_users={users_per_case} "
    ...
    *,
    ...
    packets = list(
        iter_evidence_packets(
            ...
            skip_incomplete_groups=skip_incomplete_groups,
            media_prepare_workers=media_prepare_workers,
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--skip-incomplete-groups", action="store_true")
    parser.add_argument("--media-prepare-workers", type=int, default=1)
    ...
    packets = prepare_evidence(
        ...
        skip_incomplete_groups=args.skip_incomplete_groups,
