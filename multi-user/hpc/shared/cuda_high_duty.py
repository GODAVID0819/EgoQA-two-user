#!/usr/bin/env python3
"""cuda_high_duty_keeper: repaired low-priority CUDA keeper for live QA jobs."""
...
class Burner:
    ...
    def burn(self, seconds: float, *, pause_file: Path | None = None) -> bool:
        """Run one bounded pulse, optionally aborting if a request starts.

        ``pause_file`` is retained as the legacy argument name because deployed
        launchers already use that contract.  The file now means "a productive
        request is active".  Aggressive idle pulses pass it as an abort signal;
        cautious request-time pulses deliberately do not.
        """
...
class KeeperMode:
    ...
    abort_on_request_start: bool


def select_keeper_mode(
    *,
    request_active: bool,
    idle_confirm_seconds: float,
    max_pulse_seconds: float,
    active_request_idle_confirm_seconds: float,
    active_request_max_pulse_seconds: float,
) -> KeeperMode:
    """Choose aggressive idle fill or conservative request-time gap fill."""

    if request_active:
        return KeeperMode(
            name="cautious_request_gap_fill",
            idle_confirm_seconds=active_request_idle_confirm_seconds,
            max_pulse_seconds=active_request_max_pulse_seconds,
            abort_on_request_start=False,
        )
    return KeeperMode(
        name="aggressive_idle_fill",
        idle_confirm_seconds=idle_confirm_seconds,
        max_pulse_seconds=max_pulse_seconds,
        abort_on_request_start=True,
    )
...
def decide_burn(
    *,
    ...
    memory_limited = reclaimable_free_gb < reserve_gb + 0.2
    if memory_limited and (
        current_total_util > idle_util_threshold
        or rolling_average_util >= target_util
        or idle_seconds < idle_confirm_seconds
        or reclaimable_free_gb < emergency_min_free_gb
    ):
        # Release any larger burner allocation while productive work is busy,
        # the target is already met, the idle gap is not yet confirmed, or the
        # device is genuinely close to exhaustion.  A confirmed idle gap with
        # several GiB still free may continue below using Burner's 1024x1024
        # fallback rather than disabling keepalive merely because the model
        # itself consumed the configured reserve.
    ...
    reason = (
        "emergency_idle_below_target"
        if memory_limited
        else "confirmed_idle_below_target"
    )
    return BurnDecision(True, reason, pulse_seconds)
...
class Controller(threading.Thread):
    ...
    def __init__(
        ...
        self.emergency_min_free = emergency_min_free_gb
        ...
        self.active_request_idle_confirm_seconds = active_request_idle_confirm_seconds
        self.active_request_max_pulse_seconds = active_request_max_pulse_seconds
    ...
    def run(self) -> None:
        ...
        last_sample = 0.0
        idle_since = None
        request_active_previous = None

        while not self._stop_event.is_set():
            current = time.time()
            request_active = bool(
                self.pause_file is not None and self.pause_file.exists()
            )
            mode = select_keeper_mode(
                request_active=request_active,
                idle_confirm_seconds=self.idle_confirm_seconds,
                max_pulse_seconds=self.max_pulse_seconds,
                active_request_idle_confirm_seconds=(
                    self.active_request_idle_confirm_seconds
                ),
                active_request_max_pulse_seconds=(
                    self.active_request_max_pulse_seconds
                ),
            )
            if request_active != request_active_previous:
                print(
                    f"[{now_hms()}] gpu:{self.gpu} | "
                    f"request_active:{'Y' if request_active else 'N'} | "
                    f"mode:{mode.name} | state_change:Y | buffers:retained | "
                    f"path:{self.pause_file or 'disabled'}",
                    flush=True,
                )
                request_active_previous = request_active
                idle_since = None
                last_sample = 0.0
            ...
            decision = decide_burn(
                ...
                idle_confirm_seconds=mode.idle_confirm_seconds,
                reclaimable_free_gb=reclaimable_free_gb,
                reserve_gb=self.reserve,
                emergency_min_free_gb=self.emergency_min_free,
                max_pulse_seconds=mode.max_pulse_seconds,
            )
            if decision.reason == "memory_guard":
                ...
                if not burner.burn(
                    decision.pulse_seconds,
                    pause_file=(
                        self.pause_file if mode.abort_on_request_start else None
                    ),
                ):
                    decision = BurnDecision(False, "productive_vlm_request_started")
            print(
                ...
                f"burning:{'Y' if decision.burn else 'N'} | "
                f"request_active:{'Y' if request_active else 'N'} | "
                f"mode:{mode.name} | reason:{decision.reason}",
                flush=True,
            )
...
def main() -> None:
    ...
    parser.add_argument("--emergency-min-free-gb", type=float, default=2.0)
    parser.add_argument("--start-used-mib", type=int, default=8192)
    parser.add_argument("--window-seconds", type=int, default=300)
    parser.add_argument("--target-margin", type=int, default=None)
    ...
    parser.add_argument("--max-pulse-seconds", type=float, default=0.8)
    parser.add_argument("--active-request-idle-confirm-seconds", type=float, default=1.0)
    parser.add_argument("--active-request-max-pulse-seconds", type=float, default=0.30)
    parser.add_argument(
        "--pause-file",
        default=os.getenv("CUDA_KEEPER_PAUSE_FILE", ""),
        help=(
            "Legacy-named request activity signal. While present, use cautious "
            "confirmed-idle micro-pulses instead of suspending the keeper."
        ),
    )
    args = parser.parse_args()
    if args.target_margin is None:
        args.target_margin = 100 - args.threshold
    if nvml is None:
    ...
    if args.max_prealloc <= 0:
        raise SystemExit("--max-prealloc must be positive")
    if args.emergency_min_free_gb <= 0:
        raise SystemExit("--emergency-min-free-gb must be positive")
    if (
        args.active_request_idle_confirm_seconds < 0
        or args.active_request_max_pulse_seconds <= 0
        or args.active_request_max_pulse_seconds > args.max_pulse_seconds
    ):
        raise SystemExit(
            "active-request idle confirmation must be nonnegative and its max "
            "pulse must be positive and no greater than the normal max pulse"
        )
    print(
        "cuda_high_duty_keeper_config"
        f" threshold={args.threshold}"
        f" target={args.threshold + args.target_margin}"
        f" window_seconds={args.window_seconds}"
        f" idle_util_threshold={args.idle_util_threshold}"
        f" idle_confirm_seconds={args.idle_confirm_seconds}"
        f" max_pulse_seconds={args.max_pulse_seconds}"
        f" active_request_idle_confirm_seconds={args.active_request_idle_confirm_seconds}"
        f" active_request_max_pulse_seconds={args.active_request_max_pulse_seconds}"
        f" reserve_gib={args.reserve}"
        f" max_prealloc_gib={args.max_prealloc}"
        f" emergency_min_free_gib={args.emergency_min_free_gb}"
        f" request_signal_file={args.pause_file or 'disabled'}",
        flush=True,
    )
    pause_file = Path(args.pause_file).resolve() if args.pause_file else None
    ...
    if any(reserve <= args.emergency_min_free_gb for reserve in reserves):
        raise SystemExit(
            "every --reserve value must be greater than --emergency-min-free-gb"
        )
    controllers = [
        Controller(
            gpu,
            ...
            emergency_min_free_gb=args.emergency_min_free_gb,
            ...
            active_request_idle_confirm_seconds=(
                args.active_request_idle_confirm_seconds
            ),
            active_request_max_pulse_seconds=(
                args.active_request_max_pulse_seconds
            ),
