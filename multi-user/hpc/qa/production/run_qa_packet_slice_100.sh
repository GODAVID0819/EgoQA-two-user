...
CUDA_KEEPER_SCRIPT="${CUDA_KEEPER_SCRIPT:-${PACKAGE_ROOT}/hpc/shared/cuda.py}"
CUDA_KEEPER_PID=""

ENV_SCRIPT="${PROJECT_ROOT}/hpc/shared/env_qwen3vl.sh"
if [[ ! -s "${ENV_SCRIPT}" ]]; then
  ENV_SCRIPT="${PACKAGE_ROOT}/hpc/shared/env_qwen3vl.sh"
fi
