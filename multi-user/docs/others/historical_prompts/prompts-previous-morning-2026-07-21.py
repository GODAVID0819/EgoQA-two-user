...
# Restored from the actual prompts recorded by the earlier generation runs.
JUDGE_CHECK_SCHEMA = {
    "status": "PASS/FAIL",
...
def judge_schema_for_check(
    ...
    *,
    ...
    """Return the scored PASS/FAIL contract used by the recorded legacy prompts.

    ``pass_fail_only`` remains in the signature for compatibility with the current
    caller, but the restored prompt template intentionally uses its historical
    quality fields.
    """
...
- Ask in a natural, informal, everyday way, like someone looking back on their own experience.
- Use first-person or shared-memory wording such as "I", "me", "my", "we", or "our".
- Do not ask questions directly to required_users[1]. For example, avoid asking questions like "what did you put into the basket?", or "what shirt did you wear today?".
- Do not name the required users in the question or options. Refer to people naturally only if the visible situation requires it.
- When describing other people seen in the videos, be more specific about their appearances so different people can be differentiated clearly.
- Make all five options multi-word, plausible, mutually exclusive, and parallel in length and style.
- When 2D gaze coordinates are available, they appear as <gaze_coordinate> values indicating the user's attended image area. You may use nearby visible objects, regions, or actions as evidence, but do not invent exact gaze-object claims if the projection is unclear.
- single_user_answerability must have one entry for each required user. The required_users[0] entry should say whether that view is sufficient or insufficient and why; the required_users[1] entry may say "sufficient because ..." if the evidence provider alone can answer.
- combined_answerability must explicitly say "sufficient because ..." and explain why the required users' videos together support exactly one option.
- Before returning, run the asker-alone test. Prefer a question where required_users[0] alone is insufficient, but do not reject merely because required_users[1] alone can answer.
"""
...
- Anchor the question in something visible to required_users[0]: an object they handled, a place they entered, a surface they looked toward, a task they were doing, a state they could not fully verify, etc.
- Ask for a related missing detail supplied by required_users[1]: object identity, location, text, count, state, placement, outcome, follow-up, another concrete visual fact, etc.
...
- Avoid fixed openings such as "After I ...", "When I ...", or "Once I ...".
- Prefer everyday wording and tone over formal language.
"""
    else:
        task_lines = [
            "Generate exactly one five-option multiple-choice question.",
...
- Independently verify the question, every answer option, and the declared correct answer against concrete visible moments or supplied metadata.
- Treat every object or action description in the generated item as an unverified claim, not as a hint about what the videos contain.
...
- PASS only when the question and declared correct answer are clearly supported by the provided videos or metadata, the asker's visible contextual anchor and the evidence provider's visible answer-bearing detail form a coherent situated relation, and no candidate claim relies on outside knowledge, invented gaze evidence, or unrelated clip stitching.
