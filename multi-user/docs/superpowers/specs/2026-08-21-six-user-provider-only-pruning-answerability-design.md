# 六用户 provider-only 剪枝与证据充分性回答性验证设计

## 目标

对当前六用户 QA 路径做两项调整：

1. 回答性 Gate 同时验证 speaker-only 与六段完整原视频；judge 不回答 MCQ，只判断给定视频是否包含足以产生回答的视觉证据。只有 speaker-only 不可答且 all-six 可答时才通过。
...
每次调用返回 JSON boolean `answerable`、简短 `reason`、`available_evidence` 和 `missing_evidence`。prompt 不提供声明的 gold answer，并禁止输出 A–E、选项文本或最终答案。Gate 按以下规则判定：

- speaker-only 返回 `answerable=true`：失败，标记 `speaker_only_answerable`。
- speaker-only 返回 `answerable=false`：speaker-only 子条件通过。
- speaker-only 缺失、不可解析、带 answer/choice 字段或 schema 不完整：失败，分别标记 `speaker_only_missing` 或 `speaker_only_unparsed`。
- all-six 返回 `answerable=true`：all-six 子条件通过。
- all-six 返回 `answerable=false`、缺失、不可解析、带 answer/choice 字段或 schema 不完整：失败，分别标记 `all_six_not_answerable`、`all_six_missing` 或 `all_six_unparsed`。
- 两个子条件均通过时，回答性 Gate 才通过；Gate 不读取或比较 `correct`。

结果保留两次评估、两次调用记录、媒体路径、耗时、`answerability_mode=evidence_sufficiency_reasoning`、`speaker_only_answerable`、`all_six_answerable` 和 `answerability_evaluated_condition_count=2`。QA formality 仍是独立阻断 Gate。
...
2. speaker-only 不可答且 all-six 可答时通过，并验证 Gate 不读取 gold answer。
3. speaker-only 可答、任一结果缺失或不可解析、输出 answer/choice 字段、all-six 不可答时分别失败。
