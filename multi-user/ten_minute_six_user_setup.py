...
from .group_relative_clip_sampling import (
    ...
    SIX_USER_DEFAULT_TEMPORAL_UNIT_SECONDS,
...
class BatchedImageEncoder:
    ...
    def encode(self, image_paths: list[Any]) -> list[list[float]]:
...
class LazyBatchedImageEncoder:
    """Load CLIP only when a persistent embedding-cache miss needs it."""
    ...
    def __init__(self, model_id: str, *, device: str, batch_size: int) -> None:
        ...
        self.device = device
        self.batch_size = int(batch_size)
        self._encoder: BatchedImageEncoder | None = None

    def encode(self, images: list[Any]) -> list[list[float]]:
        if self._encoder is None:
            started = time.monotonic()
            print(
                "ten_minute_prepare status=encoder_loading_on_cache_miss "
                f"model_id={self.model_id} device={self.device}",
                flush=True,
            )
            self._encoder = BatchedImageEncoder(
                TransformersClipEncoder(self.model_id, device=self.device),
                batch_size=self.batch_size,
            )
            print(
                "ten_minute_prepare status=encoder_loaded "
                f"model_id={self.model_id} seconds={time.monotonic() - started:.3f}",
                flush=True,
            )
        return self._encoder.encode(images)


def iter_double_buffered(iterable: Iterable[Any]) -> Iterable[Any]:
    """Prepare window B while the caller analyzes window A."""

    sentinel = object()
    iterator = iter(iterable)
    with ThreadPoolExecutor(
        max_workers=1, thread_name_prefix="egoqa-window-prefetch"
    ) as executor:
...
def six_user_context_plan(
    *,
    ...
    return {
        ...
        "generator_frame_policy": "complete_full_unpruned_sampled_frames",
        "generator_frame_count_is_data_dependent_after_provider_pruning": False,
        ...
        clips.append(
            {
                ...
                "temporal_pruning": {
                    ...
                    "analysis_pruning_applied_to_generator": False,
                    "analysis_source_sampled_frame_count": 3,
                },
            }
        )
    return {
...
def validate_prompt_contract(packet: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    for question_type in ("neutral", "commonality", "difference"):
        ...
        if "every one-per-second sampled frame from each" not in prompt:
            raise RuntimeError(
                f"{question_type} six-user prompt omitted full unpruned generator media"
            )
        if "samples at one frame per second" not in prompt:
            raise RuntimeError(
                f"{question_type} six-user prompt omitted the pruning sample rate"
            )
        if "globally clusters each 600-second video at K=240" not in prompt:
            raise RuntimeError(
        ...
        if "complete_full_unpruned_sampled_frames" not in prompt:
            raise RuntimeError(
...
def prepare_ten_minute_six_user(
    *,
    ...
    window_double_buffer: bool = True,
    assemble_long_video: bool = False,
    defer_gaze_summary: bool = True,
    direct_segment_rgb: bool = True,
    temporal_kmeans_time_weight: float = SIX_USER_DEFAULT_TEMPORAL_KMEANS_TIME_WEIGHT,
    ...
    if media_prepare_workers <= 0:
        raise ValueError("media_prepare_workers must be positive")
    if temporal_kmeans_time_weight < 0:
        raise ValueError("temporal_kmeans_time_weight must be non-negative")
    ...
    print(
        "ten_minute_prepare status=encoder_configured_lazy "
        f"model_id={encoder.model_id} device={device} batch_size={clip_batch_size}",
        flush=True,
    )
    ...
    source_packet_iterator = iter_evidence_packets(
        ...
        assemble_long_video=assemble_long_video,
        defer_gaze_summary=defer_gaze_summary,
        extract_preview_frames=not direct_segment_rgb,
        manifest_cache_dir=cache_dir,
    )
    if window_double_buffer:
        source_packet_iterator = iter_double_buffered(source_packet_iterator)
    ...
    while len(candidates) < target_count:
        ...
        try:
            result = analyze_group_relative_similarity(
                ...
                frame_cache_dir=Path(cache_dir) / "sampled_frames_v2",
                embedding_cache_dir=Path(cache_dir) / "clip_embeddings_v2",
                temporal_kmeans_time_weight=temporal_kmeans_time_weight,
        ...
        for candidate_result in result.get("speaker_candidates") or []:
            ...
            packet["speaker_consensus_pruning"]["diagnostics_path"] = str(
                report_path
            )
            ...
            if (
                ...
                raise RuntimeError(
                    "candidate omitted the full unpruned generator-media policy"
                )
            ...
            if context_budget.get("per_user_source_frame_counts") != context_budget.get(
                "per_user_model_input_frame_counts"
            ):
                raise RuntimeError(
                    "candidate pruned or thinned generator frames after analysis sampling"
                )
            for clip in packet.get("clips") or []:
                frames = list(clip.get("frames") or [])
                pruning = clip.get("temporal_pruning") or {}
                if (
                    clip.get("generator_media_mode")
                    != "full_unpruned_sampled_frames_only"
                    or clip.get("is_pruned") is not False
                    or pruning.get("analysis_pruning_applied_to_generator") is not False
                    or int(pruning.get("analysis_source_sampled_frame_count") or 0)
                    != len(frames)
                ):
                    raise RuntimeError(
                        "candidate generator media is not the complete full unpruned sample set"
                    )
    ...
    summary = {
        ...
        "acceleration": {
            ...
            "candidate_diagnostics": "single_external_report_plus_compact_references",
            "deferred_gaze_summary": defer_gaze_summary,
            "persistent_manifest_media_frame_embedding_cache": True,
            "clip_model_loaded_only_on_embedding_cache_miss": True,
            "assembled_long_video_materialized": assemble_long_video,
            "direct_ordered_source_segment_rgb": direct_segment_rgb,
            "intermediate_png_written": not direct_segment_rgb,
            "final_generator_frame_cache_format": (
                "lossless_png" if direct_segment_rgb else "legacy_png"
            ),
            "six_user_clip_batching": "flattened_across_users",
            "clusters_computed_once_per_video_and_reused_for_all_askers": True,
            "full_judge_video_materialization": (
                "legacy_assembled_video"
                if assemble_long_video
                else "none_ordered_source_segments_reused"
            ),
            "per_user_source_segment_map_reduce_judges": True,
            "media_pipeline_preserves_pruning_inputs": True,
            "pruning_contract_changed": temporal_kmeans_time_weight > 0,
            "validated_temporal_sidecar_promoted": temporal_kmeans_time_weight > 0,
        },
        "time_aware_pruning": {
            "temporal_policy": SIX_USER_TIME_AWARE_TEMPORAL_POLICY,
            "analysis_sample_fps": (
            ...
            "clustering_scope": (
                "full_600_seconds_global_temporal_kmeans"
                if temporal_kmeans_time_weight > 0
                else "legacy_fixed_windows"
            ),
            "global_cluster_count": int(
                math.ceil(
                    LONG_CONTEXT_EVIDENCE_DURATION_SECONDS
                    / SIX_USER_DEFAULT_CLUSTER_WINDOW_SECONDS
                    * pruning_clusters_per_window
                )
            ),
            "temporal_kmeans_time_weight": temporal_kmeans_time_weight,
            "temporal_unit_seconds": temporal_unit_seconds,
            ...
            "time_gate_applied_before_cosine": True,
            "mutual_nearest_only": False,
            "split_noncontiguous_clusters": True,
            "max_cluster_member_gap_seconds": (
                SIX_USER_DEFAULT_MAX_CLUSTER_MEMBER_GAP_SECONDS
        ...
        "media_routing": {
            "generator": "complete 1 FPS samples from all six full unpruned videos",
            "groundedness": (
...
def build_parser() -> argparse.ArgumentParser:
    ...
    prepare.add_argument(
        ...
        default=3,
        help=(
            "Maximum videos sampled concurrently with independent ffmpeg processes "
            "inside one synchronized six-user window (default: 3)"
        ),
    )
    prepare.add_argument(
        ...
        default=3,
        help=(
            "Maximum users downloaded, assembled, and probed concurrently "
            "inside one synchronized window (default: 3)"
        ),
    )
    prepare.add_argument(
    ...
    prepare.add_argument(
        ...
        default=True,
        help="Prefetch window B while CLIP and pruning analyze window A",
    )
    prepare.add_argument(
        "--assemble-long-video",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Legacy fallback; production reads the ordered 30-second segments directly",
    )
    prepare.add_argument(
        "--defer-gaze-summary",
        action=argparse.BooleanOptionalAction,
    ...
    prepare.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Pipe decoded RGB frames directly to the flattened CLIP batch",
    )
    prepare.add_argument(
        "--temporal-kmeans-time-weight",
        type=float,
        default=SIX_USER_DEFAULT_TEMPORAL_KMEANS_TIME_WEIGHT,
    )
    prepare.add_argument(
        "--temporal-unit-seconds",
        type=float,
        default=SIX_USER_DEFAULT_TEMPORAL_UNIT_SECONDS,
    )
    prepare.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "prepare":
        summary = prepare_ten_minute_six_user(
            ...
            window_double_buffer=args.window_double_buffer,
            assemble_long_video=args.assemble_long_video,
            defer_gaze_summary=args.defer_gaze_summary,
            direct_segment_rgb=args.direct_segment_rgb,
