...
class SpeakerConsensusPruningTests(unittest.TestCase):
    ...
    def test_frame_budget_keeps_every_full_unpruned_sample_and_rejects_thinning(self) -> None:
        counts = [600, 600, 600, 600, 600, 600]
        clips = [
            {
        ...
        self.assertEqual(summary["source_frame_count"], 3_600)
        self.assertEqual(summary["model_input_frame_count"], 3_600)
        self.assertEqual(summary["policy"], "complete_full_unpruned_sampled_frames")
        self.assertEqual(
        ...
        with self.assertRaisesRegex(ValueError, "do not lower the 1 FPS generator sampling rate"):
            apply_six_user_generator_frame_budget(
...
class SixUserRoleSelectionTests(unittest.TestCase):
    ...
    def test_consensus_candidate_routes_full_unpruned_frames_for_all_users(self) -> None:
        consensus = {
            "passed": True,
        ...
        self.assertEqual(
            [clip["media_role"] for clip in clips],
            [
                "speaker_full_unpruned_sampled_frames",
                *(["provider_full_unpruned_sampled_frames"] * 5),
            ],
        )
        self.assertEqual([clip["is_pruned"] for clip in clips], [False] * 6)
        self.assertEqual(
        ...
        self.assertEqual([len(clip["frames"]) for clip in clips], [4] * 6)
        self.assertEqual(
        ...
        self.assertTrue(
            all(
                [frame["sampled_frame_index"] for frame in clip["frames"]]
                == [0, 1, 2, 3]
                for clip in clips[1:]
            )
        )
        self.assertTrue(
            all(
                [
                    frame["cluster_marked_for_analysis_pruning"]
    ...
    def test_six_user_analysis_visits_all_speakers_and_keeps_every_success(self) -> None:
        ...
        self.assertEqual(len(encoder.calls[0]), 6)
