...
class TenMinuteSixUserSetupTests(unittest.TestCase):
    ...
    def test_representative_packet_uses_full_unpruned_frames_only(self) -> None:
        ...
        self.assertTrue(all(not clip["is_pruned"] for clip in clips))
        self.assertEqual(
            [clip["generator_media_mode"] for clip in clips],
            ["full_unpruned_sampled_frames_only"] * 6,
        )
    ...
    def test_prepare_wires_exact_time_aware_pruning_and_sampling_contract(self) -> None:
        ...
        # Window B is prefetched before window A is yielded for analysis.
        self.assertEqual(source_iterator.next_calls, 2)
        ...
        self.assertTrue(acceleration["processed_windows_filtered_before_download_assembly_and_clip"])
        self.assertTrue(acceleration["lazy_source_window_preparation"])
        self.assertTrue(acceleration["window_double_buffer"])
        self.assertTrue(acceleration["deferred_gaze_summary"])
        self.assertTrue(acceleration["persistent_manifest_media_frame_embedding_cache"])
        self.assertFalse(acceleration["assembled_long_video_materialized"])
        self.assertTrue(acceleration["direct_ordered_source_segment_rgb"])
        self.assertFalse(acceleration["intermediate_png_written"])
        self.assertEqual(acceleration["six_user_clip_batching"], "flattened_across_users")
        self.assertEqual(
            acceleration["full_judge_video_materialization"],
            "none_ordered_source_segments_reused",
        )
        self.assertTrue(acceleration["validated_temporal_sidecar_promoted"])
        self.assertEqual(
            summary["media_routing"]["generator"],
            "complete 1 FPS samples from all six full unpruned videos",
        )
