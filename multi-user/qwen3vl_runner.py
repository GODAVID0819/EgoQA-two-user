...
_CUDA_KEEPER_REQUEST_LOCK = threading.Lock()
_CUDA_KEEPER_REQUEST_COUNTS: dict[Path, int] = {}


@contextmanager
def cuda_keeper_signals_local_request(*, enabled: bool) -> Iterator[None]:
    """Signal that at least one local VLM HTTP request is active.

    Generator and judge runners share this process-wide, reference-counted
    signal.  Concurrent HTTP requests keep the file present until the final
    request finishes.  The keeper interprets the signal as a request to use
    confirmed-idle, short, low-priority pulses rather than fully suspending.

    ``CUDA_KEEPER_PAUSE_FILE`` remains the environment variable name for
    compatibility with already-deployed launch scripts.
    """

    raw_path = os.getenv("CUDA_KEEPER_PAUSE_FILE", "").strip()
    if not enabled or not raw_path:
        yield
        return
    signal_path = Path(raw_path).resolve()
    with _CUDA_KEEPER_REQUEST_LOCK:
        active = _CUDA_KEEPER_REQUEST_COUNTS.get(signal_path, 0)
        if active == 0:
            signal_path.parent.mkdir(parents=True, exist_ok=True)
            signal_path.write_text(
                f"pid={os.getpid()} reason=local_vlm_request\n",
                encoding="utf-8",
            )
            print(
                f"cuda_keeper_request_active active_requests=1 path={signal_path}",
                flush=True,
            )
        _CUDA_KEEPER_REQUEST_COUNTS[signal_path] = active + 1
    try:
        yield
    finally:
        with _CUDA_KEEPER_REQUEST_LOCK:
            remaining = _CUDA_KEEPER_REQUEST_COUNTS.get(signal_path, 1) - 1
            if remaining <= 0:
                _CUDA_KEEPER_REQUEST_COUNTS.pop(signal_path, None)
                signal_path.unlink(missing_ok=True)
                print(
                    "cuda_keeper_request_inactive "
                    f"active_requests=0 path={signal_path}",
                    flush=True,
                )
            else:
                _CUDA_KEEPER_REQUEST_COUNTS[signal_path] = remaining


# Compatibility alias for callers deployed with the first request-signal patch.
cuda_keeper_paused_for_local_request = cuda_keeper_signals_local_request
    ...
    def _generate_response(
        ...
        *,
        ...
        if decoding_mode == "sampling":
            ...
            if top_k is not None:
                payload["top_k"] = top_k
            raw_seed = os.getenv("VLLM_SAMPLING_SEED")
            if raw_seed is not None:
                request_seed = int(raw_seed)
                if not 0 <= request_seed < 2**32:
                    raise ValueError(
                        "VLLM_SAMPLING_SEED must be between 0 and 2**32 - 1"
                    )
                # The offline vLLM runner already forwards this seed through
                # SamplingParams.  Forward it through the OpenAI-compatible
                # request as well so concurrent arrival order cannot select a
                # different random stream for the same sampled prompt.
        ...
        print(
            ...
            f"effective_max_pixels={effective_max_pixels} "
            f"prompt_chars={len(prompt)} decoding_mode={decoding_mode} "
            f"seed={payload.get('seed', 'none')} "
            f"local_media_uris={str(self.use_local_media_file_uris).lower()}",
            flush=True,
        )
        try:
            with cuda_keeper_signals_local_request(
                enabled=self.uses_async_multimodal_frontend
            ):
