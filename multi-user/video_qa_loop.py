...
MAX_SAFE_PACKETS_IN_FLIGHT = 8
MAX_SAFE_GENERATION_LANES = 6
MAX_SAFE_REVIEW_LANES = 7
...
def six_user_role_metadata(
    ...
    for clip_index, clip in enumerate(packet.get("clips") or []):
        ...
        if clip.get("is_pruned") is not False:
            raise ValueError("six-user generator media must not be marked as pruned")
        if context_sampling.get("policy") != "complete_full_unpruned_sampled_frames":
            raise ValueError("six-user generator context policy must preserve full samples")
        if int(context_sampling.get("source_frame_count") or 0) != len(frames):
            raise ValueError("six-user generator source-frame count does not match inputs")
        if int(context_sampling.get("model_input_frame_count") or 0) != len(frames):
            raise ValueError("six-user generator input was pruned or thinned")
...
def generator_decode_config(
    *,
    ...
    configured_seed = os.getenv("VLLM_SAMPLING_SEED")
    return {
        ...
        "seed": (
            int(configured_seed)
            if generator_decode_mode == "sampling" and configured_seed is not None
            else None
        ),
    }
...
def _run_bounded_packet_pipeline(
    ...
    *,
    ...
    """Run a bounded packet pipeline with bounded generation and review lanes."""

    width = int(call_args["max_packets_in_flight"])
    generation_lanes = int(call_args["max_generation_lanes"])
    ...
    generation_gate = BoundedSemaphore(value=generation_lanes)
    ...
    print(
        ...
        f"generation_lanes={generation_lanes} review_lanes={review_lanes}",
        flush=True,
    )
...
def generate_video_qa_loop(
    *,
    ...
    if not 1 <= max_packets_in_flight <= MAX_SAFE_PACKETS_IN_FLIGHT:
        raise ValueError(
            "max_packets_in_flight must be between 1 and 8 for the bounded VLM pipeline"
        )
    if not 1 <= max_generation_lanes <= min(
        MAX_SAFE_GENERATION_LANES,
        max_packets_in_flight,
    ):
        raise ValueError(
            "max_generation_lanes must be between 1 and min(6, "
            "max_packets_in_flight)"
        )
    ...
    if not 1 <= max_review_lanes <= min(
        ...
        raise ValueError(
            "max_review_lanes must be between 1 and min(7, "
    ...
    for packet_index, packet in enumerate(iter_jsonl(evidence_path)):
        ...
        if any(
            ...
            gaze_started = time.monotonic()
            packet = hydrate_deferred_gaze_summaries(packet)
            print(
                "qa_lazy_gaze_summary "
                f"evidence_id={evidence_id} "
                f"seconds={time.monotonic() - gaze_started:.3f}",
                flush=True,
            )
...
def add_video_loop_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        default=1,
        help=(
            "Bounded packet pipeline depth, up to 8 with the async "
            "openai-compatible-local vLLM server."
        ),
    )
    parser.add_argument(
        "--max-generation-lanes",
        type=int,
        default=1,
        help=(
            "Maximum local generator requests allowed concurrently, up to 6 "
            "with the async openai-compatible-local vLLM server."
        ),
    )
    parser.add_argument(
        "--max-review-lanes",
        ...
        default=1,
        help=(
            "Maximum packet review stages allowed concurrently. Packet depth "
            "still bounds the total number of active packet workers."
        ),
    )
