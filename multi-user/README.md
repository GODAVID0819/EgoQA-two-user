...
## Offline six-user RLHF evidence preprocessing

`rlhf_evidence_preprocessing.py` builds a reusable, generator-free evidence
dataset from synchronized ten-minute, six-user windows. It samples every user
at 0.5 FPS into JPEG quality 95, encodes the persisted JPEGs once with CLIP,
and runs time-aware spherical K-means independently over each user's entire
video. The default density is 6 clusters per 30 seconds (120 global clusters
per user), with `w=0.1` and a 30-second temporal unit. It does not split a
cluster into temporal components afterward.

For each packet, the preprocessor writes all six asker conditions. An asker
always retains all 300 of their own sampled frames; each of the five providers
is pruned against that asker using a hard +/-30-second representative-time
gate, cosine threshold 0.82, and a 40% minimum retention floor. Frames and
embeddings are stored once, while the six views are represented by a compressed
`[asker, user, frame]` keep-mask tensor. All saved media paths are relative to
the packet, so a completed dataset can be moved without rewriting it.

From the cluster project root, submit the measured 256/6/6 preprocessing
profile. The default 120-packet cohort is ordered as all 90 wall-clock-aligned,
non-overlapping windows first, followed by 30 complete six-user sliding windows
chosen deterministically to maximize previously uncovered 30-second source
segments and minimize overlap with already selected windows:

```bash
cd /scratch/$USER/Long-video-understanding-clip
mkdir -p hpc/logs
sbatch --export=ALL,QWEN3VL_PROJECT_ROOT="$PWD" \
  hpc/qa/production/run_six_user_rlhf_evidence_preprocessing.sbatch
```

The persistent dataset defaults to
`/scratch/$USER/egolife_rlhf_evidence_v1`. Completed matching packets are
skipped on rerun. A stale packet is never deleted automatically; pass
`OVERWRITE_STALE=1` to archive it under `stale/` before rebuilding. Use the
`validate`, `resolve-view`, and `finalize-index` subcommands for integrity
checks, generation-time frame resolution, and rebuilding `index.jsonl`.
