...
# This file is intentionally named cuda.py, but recent PyTorch builds import
# NVIDIA's separate ``cuda.bindings`` package during ``import torch``. When this
# file is executed by path, Python normally puts its hpc/ directory first on
# sys.path, causing that import to resolve back to this file as module ``cuda``
# and creating a circular import. The keeper has no local-module imports, so it
# is safe to remove only its own directory before importing PyTorch.
_SCRIPT_DIR = os.path.realpath(os.path.dirname(__file__))
_CURRENT_DIR = os.path.realpath(os.getcwd())
sys.path[:] = [
    entry
    for entry in sys.path
    if os.path.realpath(entry or _CURRENT_DIR) != _SCRIPT_DIR
]
...
try:
    ...
    print(
        "Install PyTorch only if it is actually absent; an existing installation may instead "
        "have a CUDA package or shared-library conflict.",
        file=sys.stderr,
    )
    raise
try:
