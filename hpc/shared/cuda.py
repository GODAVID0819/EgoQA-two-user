...
# This helper is intentionally named ``cuda.py``. When Python executes it by
# path, its directory is inserted at sys.path[0]. Recent PyTorch releases import
# NVIDIA's ``cuda.bindings`` package during initialization, so leaving this
# directory on sys.path makes that import resolve back to this file and creates
# a partially initialized circular import. Remove only this script directory;
# the project root and the environment's site-packages remain available.
_SCRIPT_DIR = os.path.realpath(os.path.dirname(__file__))
sys.path[:] = [
    entry
    for entry in sys.path
    if os.path.realpath(entry or os.getcwd()) != _SCRIPT_DIR
]
...
try:
    ...
    print(f"Failed to import PyTorch: {exc!r}", file=sys.stderr); raise
try:
    import psutil
except Exception:
    psutil = None
