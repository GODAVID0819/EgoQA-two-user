...
def _nvml_handle_from_token(token: str):
    if token.isdigit():
    ...
    if token.startswith(("GPU-", "MIG-")):
        try:
            ...
            return nvml.nvmlDeviceGetHandleByUUID(token.encode("ascii"))
    raise RuntimeError(f"unsupported Slurm GPU token: {token}")


def _nvml_handle_for_logical_device(logical_index: int):
    """Map a Torch logical device to the physical NVML allocation."""
    ...
    get_nvml_index = getattr(torch.cuda, "_get_nvml_device_index", None)
    if callable(get_nvml_index):
        add("torch.cuda._get_nvml_device_index", str(int(get_nvml_index(logical_index))))
    visible = [
        token.strip()
    ...
    if logical_index < len(visible):
        add("CUDA_VISIBLE_DEVICES", visible[logical_index])
    for variable in ("SLURM_STEP_GPUS", "SLURM_JOB_GPUS"):
        allocated = [
            token.strip()
            for token in os.environ.get(variable, "").split(",")
            if token.strip()
        ]
        if logical_index < len(allocated):
            add(variable, allocated[logical_index])
    add("logical_cuda_index", str(logical_index))

    invalid_argument = getattr(nvml, "NVMLError_InvalidArgument", None)
    for source, token in candidates:
        try:
            handle = _nvml_handle_from_token(token)
        except Exception as error:
            if not (
                isinstance(invalid_argument, type)
                and isinstance(error, invalid_argument)
            ):
                raise
            continue
        return handle, source, token
    raise RuntimeError(
        f"cannot map logical CUDA device {logical_index}; "
...
class NV:
    def __init__(self, index: int) -> None:
        nvml.nvmlInit()
        self.h, source, token = _nvml_handle_for_logical_device(index)
        self._last_proc_ts = 0
        uuid = nvml.nvmlDeviceGetUUID(self.h)
        if isinstance(uuid, bytes):
            uuid = uuid.decode("ascii", errors="replace")
        print(
            "cuda_keeper_device_mapping "
            f"logical_cuda_index={index} source={source} token={token} "
            f"nvml_uuid={uuid}",
            flush=True,
        )
