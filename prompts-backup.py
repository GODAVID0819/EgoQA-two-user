...
DISCOVERED_RELATION_SCHEMA = {
    "information_needs": [
        {
            "need": "natural first-person information need someone in the situation might have",
            "speaker_user": "user whose own experience anchors the question",
            "other_required_users": ["users whose views add missing information"],
            ...
            "only_clear_when_combining": "the relation or answer target that requires multiple users",
    ...
    "selection_reason": "why this selected relation is more natural and less single-user-answerable than the alternatives",
}
...
- Treat CLIP as a pointer to candidate moments where the two users may see different things.
...
def build_relation_discovery_prompt(
    ...
    type_hint = {
        "commonality": (
            "Prefer a relation where one user's anchor and another user's missing detail jointly establish "
...
Then select exactly one relation that is natural, visually grounded, and least likely to be answerable by one user's video alone.
...
def build_relation_mcq_prompt(
    ...
    type_instruction = {
        "commonality": (
            "The relation should become a question whose answer is only clear after combining the required users' views."
        ),
        "difference": (
            "The relation should become a question about a meaningful asymmetry or complementary detail."
        ),
    }[question_type]
    return f"""You are writing one natural first-person EgoLife MCQ from a discovered cross-user relation.

Use this discovered relation to write one natural first-person MCQ.
