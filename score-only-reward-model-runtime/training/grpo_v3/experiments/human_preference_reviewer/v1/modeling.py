"""Rank-consistent cumulative ordinal heads for Reviewer v1."""
...
OUTPUT_FIELDS = (
    "evidence_threshold_logits",
    "answerability_threshold_logits",
    "formality_threshold_logits",
)
HEAD_MODULES = {
...
class ReviewerOutput:
    evidence_threshold_logits: Any | None = None
    answerability_threshold_logits: Any | None = None
    formality_threshold_logits: Any | None = None
...
if nn is not None:
    class OrdinalHead(nn.Module):
        """One latent score with two strictly ordered cumulative thresholds."""
        ...
        def __init__(self, hidden_size: int) -> None:
            ...
            self.threshold_start = nn.Parameter(torch.zeros(()))
            initial_gap = math.log(math.expm1(1.0))
            self.threshold_gap_unconstrained = nn.Parameter(torch.tensor(initial_gap))

        def ordered_thresholds(self) -> Any:
            gap = torch.nn.functional.softplus(self.threshold_gap_unconstrained) + 1e-6
            return torch.stack((self.threshold_start, self.threshold_start + gap))

        def forward(self, inputs: Any) -> Any:
            return self.score(inputs) - self.ordered_thresholds()


    class ReviewerV1(nn.Module):
        """Wrap Qwen3-VL with independent rank-consistent ordinal score heads."""
        ...
        def __init__(
            ...
            for module_name in head_module_names(self.active_heads):
                setattr(self, module_name, OrdinalHead(hidden_size))
        ...
        def forward(self, **inputs: Any) -> ReviewerOutput:
            ...
            head_input = pooled.to(dtype=first_head.score.weight.dtype)
            return ReviewerOutput(
                evidence_threshold_logits=(
                    self.evidence_head(head_input) if hasattr(self, "evidence_head") else None
                ),
                answerability_threshold_logits=(
                    self.answerability_head(head_input) if hasattr(self, "answerability_head") else None
                ),
                formality_threshold_logits=(
                    self.formality_head(head_input) if hasattr(self, "formality_head") else None
                ),
                pooled_hidden=pooled,
            )
else:
    class OrdinalHead:  # type: ignore[no-redef]
        def __init__(self, *_: Any, **__: Any) -> None:
            raise RuntimeError("PyTorch is required to construct OrdinalHead")
